...
def write_structured_report(report_data: dict, filename: str = "variant_classification_report.txt"):
    ...
    with open(filename, "a", encoding="utf-8") as f:
        f.write("\n" + "="*60 + "\n")
        f.write(f"ACMG VARIANT CLASSIFICATION REPORT\n")
        f.write("="*60 + "\n")
        f.write(f"Date/Time: {report_data.get('date', time.strftime('%Y-%m-%d %H:%M'))}\n")
        f.write(f"Gene: {report_data.get('gene', 'N/A')}\n")
        f.write(f"Chromosome: {report_data.get('chromosome', 'N/A')}\n")
        f.write(f"Position: {report_data.get('position', 'N/A')}\n")
        f.write(f"Reference Allele: {report_data.get('ref_allele', 'N/A')}\n")
        f.write(f"Alternate Allele: {report_data.get('alt_allele', 'N/A')}\n")
        f.write(f"cDNA Change: {report_data.get('cdna_change', 'N/A')}\n")
        f.write(f"Protein Change: {report_data.get('protein_change', 'N/A')}\n")
        f.write(f"VEP Consequence: {report_data.get('vep_consequence', 'N/A')}\n")
        f.write(f"Varsome URL: {report_data.get('varsome_url', 'N/A')}\n")
        f.write(f"ClinVar Status: {report_data.get('clinvar_status', 'N/A')}\n")
        f.write("-"*60 + "\n")
        f.write(f"Allele Frequency: {report_data.get('allele_frequency', 'N/A')}\n")
        f.write(f"Disease Prevalence: {report_data.get('disease_prevalence', 'N/A')}\n")
        if 'subpopulation_frequencies' in report_data:
            subpops = report_data.get('subpopulation_frequencies', [])
            if subpops:
                f.write(f"Subpopulation Frequencies: {', '.join(str(x) for x in subpops)}\n")
        f.write("-"*60 + "\n")
        f.write("In Silico Scores:\n")
        for tool, score in report_data.get('insilico_scores', {}).items():
            f.write(f"  {tool.upper()}: {score}\n")
        f.write(f"Combined MetaScore: {report_data.get('metascore', 'N/A')}\n")
        if 'criteria' in report_data:
            f.write(f"Applied ACMG Criteria: {', '.join(report_data.get('criteria', []))}\n")
        if 'classification' in report_data:
            f.write(f"Final Classification: {report_data.get('classification', 'N/A')}\n")
        if 'fisher_result' in report_data:
            f.write(f"Fisher's Exact Test: {report_data.get('fisher_result', 'N/A')}\n")
        if 'outlier_result' in report_data:
            f.write(f"Subpopulation Outlier: {report_data.get('outlier_result', 'N/A')}\n")
        if 'insilico_prediction' in report_data:
            f.write(f"In Silico Prediction: {report_data.get('insilico_prediction', 'N/A')}\n")
        if 'notes' in report_data:
            f.write(f"Notes: {report_data.get('notes', '')}\n")
        f.write("="*60 + "\n")
        f.write("LICENSE: MIT (c) Can Sevilmiş\n")
        f.write("="*60 + "\n")
...
def collect_insilico_inputs(variant_type, variant, gene):
    ...
    # This function should return a dict for update() compatibility
    return {}

# --- Main ACMG Assessment and Report Saving ---
def run_acmg_assessment():
    """Main entry point for ACMG assessment and structured report saving."""
    # Collect all user inputs (basic info, population, in silico, etc.)
    ...
    # In silico scores
    insilico_scores = {}
    for tool in GENE_SPECIFIC_WEIGHTS.get(variant['gene'], GENE_SPECIFIC_WEIGHTS['default']).keys():
    ...
    variant['criteria'] = [c for c in [criterion] if c]
    # Example: add more ACMG criteria as needed
    variant['classification'] = 'VUS'  # Placeholder, replace with actual logic
    variant['notes'] = ''

    # ClinVar and Varsome
    clinvar_status, clinvar_note = get_clinvar_status(variant['chromosome'], variant['position'], variant['ref_allele'], variant['alt_allele'])
    variant['clinvar_status'] = clinvar_status
    variant['varsome_url'] = generate_varsome_url(variant['chromosome'], variant['position'], variant['ref_allele'], variant['alt_allele'])

    # Add date
    variant['date'] = time.strftime('%Y-%m-%d %H:%M')

    # Save structured report
    write_structured_report(variant)
    print("\nStructured ACMG report saved.")
    print("[INFO] Report file: variant_classification_report.txt (in the current folder)")
