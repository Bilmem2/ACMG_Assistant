...
# Move data collection functions above this point to avoid NameError
...
def run_acmg_assessment():
    ...
    conservation_tools = [
        ('phylop_vert', 'phyloP Vertebrate: Conservation score (-20 to 20). High = conserved.'),
        ('phylop_mamm', 'phyloP Mammal: Conservation score (-20 to 20). High = conserved.'),
        ('phylop_primate', 'phyloP Primate: Conservation score (-20 to 20). High = conserved.'),
    ...
    # Simple ACMG classification logic based on metascore (for demonstration)
    if metascore > 0.354:
        acmg_class = 'Likely Pathogenic or Pathogenic (PP3)'
    elif metascore < 0.226:
        acmg_class = 'Likely Benign or Benign (BP4)'
    else:
        acmg_class = 'VUS (Uncertain Significance)'
    variant['classification'] = acmg_class
    variant['notes'] = ''
    ...
    # Save structured report only after all processing is complete
    try:
        print("[DEBUG] About to write ACMG report...")
        # Merge top-level notes and varsome_url from inputs if available
        report_data = variant.copy()
        import inspect
        ...
        while frame:
            if 'inputs' in frame.f_locals:
                ...
                if isinstance(inputs, dict):
                    if 'notes' in inputs:
                        report_data['notes'] = inputs['notes']
                    if 'varsome_url' in inputs:
                        report_data['varsome_url'] = inputs['varsome_url']
                break
            frame = frame.f_back
        write_structured_report(report_data)
        print("[DEBUG] Report writing function completed.")
        print("\nStructured ACMG report saved.")
        print("[INFO] Report file: variant_classification_report.txt (in the current folder)")
        # Print terminal-only warnings and suggestions after report generation
        print_terminal_warnings_and_suggestions(report_data)
    except Exception as e:
        import traceback
        print("\n[ERROR] Failed to save ACMG report:", str(e))
